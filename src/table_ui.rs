...
use crate::entities::{TableTab, Row, Edge, Glyph, GlyphShape, Mesh, generate_edge_id, remove_dangling_edges};
use crate::csv_io::{import_nodes_csv, export_nodes_csv, import_edges_csv, export_edges_csv};
                        ...
                        TableTab::Nodes => {
                            ...
                            if ui.button(format!("{} Import CSV", egui_phosphor::regular::DOWNLOAD_SIMPLE)).clicked() {
                                if import_nodes_csv(rows, channel_names) {
                                    remove_dangling_edges(rows, edges);
                                    changed = true;
                                }
                            }
                        }
        ...
        .body(|mut body| {
            ...
            for i in 0..n {
                ...
                body.row(row_h, |mut row_ui| {
                    ...
                    // X Y Z
                    row_ui.col(|ui| { if float_input(ui, &mut rows[i].x_str) { changed = true; } });
                    row_ui.col(|ui| { if float_input(ui, &mut rows[i].y_str) { changed = true; } });
                    row_ui.col(|ui| { if float_input(ui, &mut rows[i].z_str) { changed = true; } });
                    // dX dY dZ rX rY rZ
                    row_ui.col(|ui| { if channel_dropdown(ui, i, 0, &mut rows[i].channel_dx, channel_names, &dropdown_label) { changed = true; } });
                    row_ui.col(|ui| { if channel_dropdown(ui, i, 1, &mut rows[i].channel_dy, channel_names, &dropdown_label) { changed = true; } });
                    row_ui.col(|ui| { if channel_dropdown(ui, i, 2, &mut rows[i].channel_dz, channel_names, &dropdown_label) { changed = true; } });
                    row_ui.col(|ui| { if channel_dropdown(ui, i, 3, &mut rows[i].channel_rx, channel_names, &dropdown_label) { changed = true; } });
                    row_ui.col(|ui| { if channel_dropdown(ui, i, 4, &mut rows[i].channel_ry, channel_names, &dropdown_label) { changed = true; } });
                    row_ui.col(|ui| { if channel_dropdown(ui, i, 5, &mut rows[i].channel_rz, channel_names, &dropdown_label) { changed = true; } });
                    // Delete
                    row_ui.col(|ui| {
