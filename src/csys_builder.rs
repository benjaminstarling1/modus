...
impl CsysOp {
    fn label(&self) -> &'static str {
        match self {
            CsysOp::RotateX(_) => "Rotate X",
            CsysOp::RotateY(_) => "Rotate Y",
            CsysOp::RotateZ(_) => "Rotate Z",
        }
    }

    fn angle_mut(&mut self) -> &mut f32 {
        match self {
            CsysOp::RotateX(a) | CsysOp::RotateY(a) | CsysOp::RotateZ(a) => a,
        }
    }
...
impl CsysBuilder {
    ...
    pub fn load_with_ops(&mut self, base: [[f32; 3]; 3], ops: Vec<CsysOp>) {
...
pub fn show_csys_builder_window(
    ctx:                 &egui::Context,
    open:                &mut bool,
    builder:             &mut CsysBuilder,
    target_label:        Option<&str>,
    on_apply:            &mut Option<([[f32; 3]; 3], [[f32; 3]; 3], Vec<CsysOp>)>,
    on_save_to_manager:  &mut Option<(String, [[f32; 3]; 3], [[f32; 3]; 3], Vec<CsysOp>)>,
) {
    if !*open { return; }
    ...
    let mut apply_result: Option<([[f32; 3]; 3], [[f32; 3]; 3], Vec<CsysOp>)>        = None;
    let mut save_mgr_result: Option<(String, [[f32; 3]; 3], [[f32; 3]; 3], Vec<CsysOp>)> = None;
            ...
            ui.horizontal_top(|ui| {
                ...
                ui.vertical(|ui| {
                                        ...
                                        egui::ComboBox::from_id_salt(("op_type", i))
                                            ...
                                            .show_ui(ui, |ui| {
                                                ...
                                                if ui.selectable_label(
                                                    matches!(builder.ops[i], CsysOp::RotateX(_)),
                                                    "Rotate X",
                                                ).clicked() {
                                                    builder.ops[i] = CsysOp::RotateX(angle);
                                                }
                                                if ui.selectable_label(
                                                    matches!(builder.ops[i], CsysOp::RotateY(_)),
                                                    "Rotate Y",
                                                ).clicked() {
                                                    builder.ops[i] = CsysOp::RotateY(angle);
                                                }
                                                if ui.selectable_label(
                                                    matches!(builder.ops[i], CsysOp::RotateZ(_)),
                                                    "Rotate Z",
                                                ).clicked() {
                                                    builder.ops[i] = CsysOp::RotateZ(angle);
                                                }
                                            });
                    ...
                    if ui.button(format!("{}  Add Operation", egui_phosphor::regular::PLUS)).clicked() {
                        builder.ops.push(CsysOp::RotateY(0.0));
                    }
...
/// Application-level list of named CSYSes.
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
pub struct CsysManager {
    pub entries: Vec<CsysEntry>,
}

impl Default for CsysManager {
    fn default() -> Self {
        Self {
            entries: vec![CsysEntry {
...
impl CsysManager {
    ...
    pub fn add_or_replace(&mut self, name: String, matrix: [[f32; 3]; 3], base_matrix: [[f32; 3]; 3], ops: Vec<CsysOp>) {
        if let Some(e) = self.entries.iter_mut().find(|e| e.name == name && !e.locked) {
            e.matrix = matrix;
            e.base_matrix = base_matrix;
            e.ops = ops;
        } else {
            self.entries.push(CsysEntry { name, matrix, base_matrix, ops, locked: false });
        }
    }
}
...
pub fn show_csys_manager_panel(
    ui:             &mut egui::Ui,
    manager:        &mut CsysManager,
    selected_count: usize,
    on_apply:       &mut Option<([[f32; 3]; 3], [[f32; 3]; 3], Vec<CsysOp>)>,
    on_edit:        &mut Option<(String, [[f32; 3]; 3], Vec<CsysOp>)>,
) {
    ui.label(egui::RichText::new("CSYS Manager").size(16.0).strong());
    ui.separator();
    ...
    let mut delete_idx: Option<usize> = None;
    let mut apply_mat:  Option<([[f32; 3]; 3], [[f32; 3]; 3], Vec<CsysOp>)> = None;
    let mut edit_entry: Option<(String, [[f32; 3]; 3], Vec<CsysOp>)> = None;
